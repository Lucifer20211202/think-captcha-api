...
class CaptchaApi
{
    ...
    public function createSMS(string $phone = null): array
    {
        ...
        $this->cache->set('captchaSMS:'.$phone, $generator['code'], $this->expire);
        return [
            'key'  => $phone,
            'code' => $generator['code'],
        ];
     ...
     * @return object
     */
    public function checkSMS(string $code, string $phone, bool $res = false): bool
    {
        $runres = false;
     ...
     */
    public function check(string $code, string $hash, bool $res = false): bool
    {
        $runres = false;
        if ($this->cache->get('captchaApi:'.$hash)) {
            $code = mb_strtolower($code, 'UTF-8');
            $runres = password_verify($code, $hash);
        }
     ...
     * @return object
     */
    public function create(string $config = null): array
    {
        ...
        // 绘验证码
        $text = $this->useZh ? preg_split('/(?<!^)(?!$)/u',
            $generator['value']) : str_split($generator['value']); // 验证码
    ...
    protected function writeCurve(): void
    {
        ...
        for ($px = $px1; $px <= $px2; $px = $px + 1) {
            if (0 != $w) {
                ...
                while ($i > 0) {
                    imagesetpixel($this->im, $px + $i, $py + $i,
                        $this->color); // 这里(while)循环画像素点比imagettftext和imagestring用字体大小一次画出（不用这while循环）性能要好很多
                    $i--;
                }
            }
        }
    ...
    protected function writeNoise(): void
    {
        ...
        for ($i = 0; $i < 10; $i++) {
            ...
            for ($j = 0; $j < 5; $j++) {
                // 绘杂点
                imagestring($this->im, 5, mt_rand(-10, (int)$this->imageW), mt_rand(-10, (int)$this->imageH),
                    $codeSet[mt_rand(0, 29)], $noiseColor);
            }
        }
    }
